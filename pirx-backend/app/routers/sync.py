    ...
    if payload.type == "auth" and payload.status == "success" and payload.user:
        ...
        for daily_payload in payload.data:
            try:
                ...
                if (
                    ...
                    and normalized.get("hrv") is None
                    and custom.get("weight_kg") is None
                    and custom.get("body_fat_percentage") is None
                    and custom.get("bmi") is None
                ):
                    skipped += 1
                    continue
                db.insert_wearable_physiology(user_id, normalized)
        ...
        if stored > 0:
            try:
                ...
                logger.exception(
                    "Failed to queue feature engineering after Terra daily for user %s",
                    user_id,
                )
...
async def recompute_pipeline(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.warning(
            "recompute_pipeline: activity_type distribution: %s",
            type_counts,
        )
